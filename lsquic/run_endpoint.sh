...
if [ "$ROLE" = server ]; then
    ...
    echo SERVER_PARAMS: LSQUIC_FORCE_RETRY=$FORCE_RETRY $SERVER_PARAMS $VERSIONS $ECN
    LSQUIC_FORCE_RETRY=$FORCE_RETRY exec /usr/bin/http_server $SERVER_PARAMS $VERSIONS $ECN \
